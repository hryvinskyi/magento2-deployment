...
**Пакет composer тихо перестав автозавантажуватися після вдалого деплою.** Пакет із `path`-репозиторію composer ставить як відносний symlink назовні з `vendor/` (`vendor/acme/sdk -> ../../composer/acme/sdk`), тож його джерело лежить поза каталогами, які копіює build-клон. До 3.2.1 у білді цей symlink вів у нікуди, і `composer dump-autoload` викидав пакет з автолоадера з нульовим кодом виходу — деплой звітував про успіх, а класи зникали. Починаючи з 3.2.1 клон переносить такі джерела, а білд падає, якщо його автолоадер реєструє менше неймспейсів, ніж живий. `path`-репозиторій, джерело якого лежить *поза* коренем Magento, перенести неможливо: запуск попереджає про це, і його треба перенести всередину кореня.
